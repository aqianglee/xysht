...
<body>
	...
	<div class="context">
		...
		<div class="right">
			...
			<c:if
				test="${classfies != null && fn:length(requestScope.classfies) > 0}">
				<ol style="margin-left: 20px">
					<c:forEach var="s" items="${requestScope.classfies }">
						<a
							href="getGoodsByClassfy?sid=${s.supermarket.id }&classfyId=${s.id}">${s.name }</a>
					</c:forEach>
				</ol>
			</c:if>
			<div class="right_title">
				<a class="btn btn-primary"
					href="${pageContext.request.contextPath }/shopKeeper/editGood?classfyId=${classfy.id}">添加商品</a>
			</div>
			<div align="center">
				<c:if
					test="${requestScope.goods != null && requestScope.goods.size() > 0 }">
					<table class="table">
						<tr>
							<th>图片</th>
							<th>商品名称</th>
							...
							<tr>
								<td>
									<div style="width: 50px; height: 50px;">
										<img alt="" height="50"
											src='<c:if test="${g.picture != null }">${pageContext.request.contextPath }/showPicture?pictureId=${g.picture.id }</c:if>
								...
								<td></td>
								<td><a
									href="${pageContext.request.contextPath }/shopKeeper/editGood?id=${g.id }&classfyId=${classfy.id }">修改</a></td>
								<td><a
									href="${pageContext.request.contextPath }/shopKeeper/deleteGood?id=${g.id}&classfyId=${classfy.id }">删除</a></td>
							</tr>
						</c:forEach>
					</table>
				</c:if>
