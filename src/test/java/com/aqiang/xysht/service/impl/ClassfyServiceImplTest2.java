...
public class ClassfyServiceImplTest2 extends BaseTest {
	...
	public void testGetClassfiesByParent() {
		...
		List<Classfy> classfies2 = classfyService.getClassfiesByParent(supermarket, clothes);
		isEq("外套", classfies2.get(0).getName());
		isEq("牛仔裤", classfies2.get(1).getName());
		isEq(2, classfies2.size());
		List<Classfy> classfies3 = classfyService.getClassfiesByParent(supermarket, snacks);
		isEq(2, classfies3.size());
		isEq("辣条", classfies3.get(0).getName());
		isEq("方便面", classfies3.get(1).getName());
	}
